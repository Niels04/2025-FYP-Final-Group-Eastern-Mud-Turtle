...
def main(csvPath:str, savePath:str, testImgDir:str, testMaskDir:str, testMetadataPath:str) -> None:
    ...
    if testImgDir is not None:
        ...
        testData = extract(testImgDir, testMaskDir, testMetadataPath, feature_dir=None, base_model=False)#could also get rid of base_model=False since we select the correct features anyways
    ...
    #NOTE: HERE is the place where we can input our code for the extended method
    #      (i.e. creating the classifier with the correct hyperparameters etc...)
    #      + select the correct features

    #select features for extended method
    xTrain = xTrain[["fA_score", "fB_score", "fC_score", "fBV_score", "fCH_score", "fS_score"]]
